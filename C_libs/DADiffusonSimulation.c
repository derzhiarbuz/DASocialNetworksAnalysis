...
double DADSLogLikelyhoodTKDR(double theta, double confirm, double decay, double relic) {
    int32_t i, j, k, n_i;
    double t = .0, dt, loglikelyhood, inf_rate, cd;
...
/*    for(i=0; i<und_network.N; i++) {
    ...
    for(;i<und_network.N_cases; i++) {
        ...
        for(j=0; j<=i; j++) {
            if(decay > .0) {
                und_network.cases[j].value1 = theta/decay*(exp(decay*(und_network.cases[j].time - t)) - exp(decay*(und_network.cases[j].time - cur_case.time)));
                und_network.cases[j].value2 = theta*exp(decay*(und_network.cases[j].time - cur_case.time));
            }
            else {
                und_network.cases[j].value1 = theta*(cur_case.time - t);
                und_network.cases[j].value2 = theta;
            }
        }
        dt = cur_case.time - t;
        t = cur_case.time;
