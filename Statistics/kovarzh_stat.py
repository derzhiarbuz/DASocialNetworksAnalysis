...
def task_3():
    ...
    tot_frm = pd.concat([avg_squared, stat_squared], axis=1)
    avg_cols = avg_squared.columns
    stat_cols = stat_squared.columns
    corr = pd.DataFrame(data=np.zeros((len(avg_cols), len(stat_cols))), columns=stat_cols, index=avg_cols)
    pval = corr.copy()

    # testing normality
    # normals = set()
    # for col in stat_cols:
    #     if stat_squared[col].isna().sum() > 4:
    #         continue
    #     nt = stats.normaltest(stat_squared[col])
    #     if nt.pvalue < 0.05:
    #         normals.add(col)
    #
    # for col in avg_cols:
    #     if avg_squared[col].isna().sum() > 4:
    #         continue
    #     nt = stats.normaltest(avg_squared[col])
    #     if nt.pvalue < 0.05:
    #         normals.add(col)
    ...
    for row in avg_cols:
        for col in stat_cols:
            r = {}
            # if row in normals and col in normals:
            #     first = avg_squared[row].fillna(avg_squared[row].mean(axis=0, skipna=True))
            #     second = stat_squared[col].fillna(stat_squared[col].mean(axis=0, skipna=True))
            #     r = stats.pearsonr(first, second)
            #     r = {'correlation': c[0], 'pvalue': c[1]}
            # elif avg_squared[row].isna().sum() == 12 and stat_squared[row].isna().sum() == 12:
            if avg_squared[row].isna().sum() == 12 and stat_squared[row].isna().sum() == 12:
                c = stats.spearmanr(avg_squared[row], stat_squared[col], nan_policy='omit')
                r = {'correlation': c.correlation, 'pvalue': c.pvalue}
            else:
                c = stats.kendalltau(avg_squared[row], stat_squared[col], nan_policy='omit')
                r = {'correlation': c.correlation, 'pvalue': c.pvalue}
            if r['pvalue'] < 0.05:
                corr[col][row] = r['correlation']
            else:
                corr[col][row] = np.NaN
        # pval[col][row] = r['pvalue']
        # print(str(c.correlation) + '   ' + str(c.pvalue))

    # corr = stats.spearmanr(avg_squared, stat_squared, axis=None, nan_policy='omit')
    ...
    #
    # fac_avg = FactorAnalysis(n_components=4, svd_method='lapack')
    ...
    #
    # fac_stat = FactorAnalysis(n_components=2, svd_method='lapack')
...
if __name__ == '__main__':
    # Четвёртая задача
    nonaz_df = pd.read_csv('D:/BigData/Galina_Kovarzh/Nenatsionalnye_23_mar.csv',
                           sep=';', decimal=',', encoding='windows-1251', index_col=0)
    #nonaz_df = nonaz_df.iloc[:n_subjects1, :]
    naz_df = pd.read_csv('D:/BigData/Galina_Kovarzh/Natsionalnye_23_mar.csv',
                         sep=';', decimal=',', encoding='windows-1251', index_col=0)
    #naz_df = nonaz_df.iloc[:n_subjects1, :]

    cols = [0, 1, 2, 15, 28, 41, 54, 67, 80, 93, 106, 119, 132, 145, 158, 171, 184, 197, 210, 223, 236,
            250, 251, 252, 253, 254, 255, 256, 257, 258, 259, 260, 261, 262, 263, 264, 265]

    nonaz_df = nonaz_df.iloc[:, cols]
    naz_df = naz_df.iloc[:, cols]

    for i in range(len(nonaz_df.columns)):
        print(str(i) + ' ' + str(nonaz_df.dtypes[i]) + ' ' + nonaz_df.columns[i])

    nonaz_df[nonaz_df.columns[21]] /= nonaz_df[nonaz_df.columns[0]]
    naz_df[naz_df.columns[21]] /= naz_df[naz_df.columns[0]]

    nonaz_df[nonaz_df.columns[22]] /= nonaz_df[nonaz_df.columns[0]]
    naz_df[naz_df.columns[22]] /= naz_df[naz_df.columns[0]]

    nonaz_df[nonaz_df.columns[25]] /= nonaz_df[nonaz_df.columns[0]]
    naz_df[naz_df.columns[25]] /= naz_df[naz_df.columns[0]]

    nonaz_df[nonaz_df.columns[26]] /= nonaz_df[nonaz_df.columns[0]]
    naz_df[naz_df.columns[26]] /= naz_df[naz_df.columns[0]]

    nonaz_df[nonaz_df.columns[28]] /= nonaz_df[nonaz_df.columns[0]]
    naz_df[naz_df.columns[28]] /= naz_df[naz_df.columns[0]]

    nonaz_df[nonaz_df.columns[30]] /= nonaz_df[nonaz_df.columns[0]]
    naz_df[naz_df.columns[30]] /= naz_df[naz_df.columns[0]]

    nonaz_df[nonaz_df.columns[32]] /= nonaz_df[nonaz_df.columns[0]]
    naz_df[naz_df.columns[32]] /= naz_df[naz_df.columns[0]]

    nonaz_df[nonaz_df.columns[34]] /= nonaz_df[nonaz_df.columns[0]]
    naz_df[naz_df.columns[34]] /= naz_df[naz_df.columns[0]]

    corr_nonaz = pd.DataFrame(data=np.zeros((len(nonaz_df.columns), len(nonaz_df.columns))),
                        columns=nonaz_df.columns, index=nonaz_df.columns)
    corr_naz = pd.DataFrame(data=np.zeros((len(naz_df.columns), len(naz_df.columns))),
                              columns=naz_df.columns, index=naz_df.columns)

    # for row in nonaz_df.columns:
    #     for col in nonaz_df.columns:
    #         c = stats.spearmanr(nonaz_df[row], nonaz_df[col], nan_policy='omit')
    #         if c.pvalue < 0.05:
    #             corr_nonaz[col][row] = c.correlation
    #         else:
    #             corr_nonaz[col][row] = np.NaN
    #
    # for row in naz_df.columns:
    #     for col in naz_df.columns:
    #         c = stats.spearmanr(naz_df[row], naz_df[col], nan_policy='omit')
    #         if c.pvalue < 0.05:
    #             corr_naz[col][row] = c.correlation
    #         else:
    #             corr_naz[col][row] = np.NaN
    #
    # print(corr_nonaz)
    # print(corr_naz)
    #
    # corr_nonaz = corr_nonaz.iloc[0:21, 21:]
    # corr_naz = corr_naz.iloc[0:21, 21:]
    #
    # corr_nonaz.to_csv('D:/BigData/Galina_Kovarzh/nonaz_corr_ucheniki_bolnie.csv')
    # corr_nonaz.to_csv('D:/BigData/Galina_Kovarzh/naz_corr_ucheniki_bolnie.csv')

    greens_trans = makefac(nonaz_df.iloc[2:21, :], 4, 'D:/BigData/Galina_Kovarzh/Nonaz_our_factors.csv', 'Green')
    print(greens_trans)